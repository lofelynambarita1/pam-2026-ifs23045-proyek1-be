...
fun Application.configureRouting() {
    val layananService: LayananService by inject()
    ...
    routing {
        get("/") {
            call.respondText("API Hairlogy telah berjalan. Dibuat oleh Lofelyn Enzely Ambarita.")
        }

        // Route Layanan
        route("/layanan") {
            get {
                layananService.getAllLayanan(call)
            }
            post {
                layananService.createLayanan(call)
            }
            get("/{id}") {
                layananService.getLayananById(call)
            }
            put("/{id}") {
                layananService.updateLayanan(call)
            }
            delete("/{id}") {
                layananService.deleteLayanan(call)
            }
            get("/{id}/gambar") {
                layananService.getLayananGambar(call)
            }
        }
